...
const BrowseAllPages = () => {
  ...
  return (
    <div className="container-fluid py-4" style={{ minHeight: "100vh" }}>
      {/* Header */}
      <div className="mb-4">
        <h2 className={`mb-2 fw-bold ${isDarkMode ? "text-white" : "text-black"}`}>Discover Pages</h2>
        <p className={`mb-0 ${isDarkMode ? "text-secondary" : "text-muted"}`}>Find and follow pages that interest you</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-5">
          <div className="mb-4">
            <i className={`fas fa-search  ${isDarkMode ? "text-secondary" : "text-muted"}`} style={{ fontSize: "4rem" }}></i>
          </div>
          <h4 className={`mb-3 ${isDarkMode ? "text-secondary" : "text-muted"}`}>No Pages Found</h4>
          <p className={`mb-0 ${isDarkMode ? "text-secondary" : "text-muted"}`}>Try adjusting your search or filters to find more pages.</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="modal-footer border-0 bg-light px-4 py-3">
                  <div className="d-flex gap-2 w-100">
                    {!page.isOwner && (
                      <button
                        className={`btn flex-grow-1 fw-medium ${page.isFollowing
                          ? 'btn-outline-danger'
                          : 'btn-primary'
                          }`}
