...
const CategoryListJobRole  = () => {
    ...
    const fetchCategories = async () => {
        try {
            setLoading(true);
            const result = await CategoryService.getAllCategories('job_role');
            if (result.success) {
    ...
    const handleToggleActive = async (categoryId, isActive) => {
        ...
        try {
            ...
            if (result.success) {
                toast.success(result.message || `Category ${isActive ? 'deactivated' : 'activated'} successfully!`);
                fetchCategories(); // Refetch to get updated data
            }
        } catch (error) {
        ...
        return (
            <div className="category-list-container">
                <div className="ct-loading-container">
                    ...
                    <p>Loading job role categories...</p>
                </div>
            </div>
        );
    }
  ...
  return (
    <div className="category-list-container">
            <div className="category-list-header">
                <h3>Job Role Categories ({categories.length})</h3>
            </div>

            {categories.length === 0 ? (
                <div className="empty-state">
                    <p>No job role categories found. Create your first category!</p>
                </div>
            ) : (
...
export default CategoryListJobRole 